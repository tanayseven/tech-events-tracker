...
**`EVENT_LIST` is the source of truth for scope.** Only events listed there are ever written or updated by `main.py`. Manually added events (entries whose name doesn't match any managed event) are preserved across re-runs via the `upsert()` function.

**Upsert strategy:** `upsert()` seeds the deduplication `seen` set only from *unmanaged* entries, so managed event events are always fully replaced by the latest fetch. Dedup key is `(name.lower(), date)`.

**`series` field:** stamped by `main.py` via `e.setdefault("series", name)` using the `EVENT_LIST` value. Recurring meetups (e.g. Expert Talks) return multiple objects from the same search, all sharing the same `series`.
